...
from webapp.shared.datex2.v3_7.shared.hydrogen_input import HydrogenInput
...
class EnergyInfrastructureStationInput(ValidataclassMixin):
    ...
    totalMaximumPower: float | UnsetValueType = FloatValidator(allow_integers=True), Default(UnsetValue)
    ...
    numberOfRefillPoints: int | UnsetValueType = IntegerValidator(min_value=0), Default(UnsetValue)
    userInterfaceLanguage: list[str] | UnsetValueType = (
        ListValidator(RegexValidator(pattern=r'^[a-z]{2}$')),
        Default(UnsetValue),
    )
    renewableSources: bool | UnsetValueType = BooleanValidator(), Default(UnsetValue)
    ...
    hydrogen: HydrogenInput | UnsetValueType = DataclassValidator(HydrogenInput), Default(UnsetValue)
