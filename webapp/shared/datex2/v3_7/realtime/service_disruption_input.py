...
from webapp.shared.datex2.v3_7.shared.service_disruption_type_enum_g_input import ServiceDisruptionTypeEnumGInput
...
@validataclass
class ServiceDisruptionInput(ValidataclassMixin):
    ...
    serviceDisruptionType: list[ServiceDisruptionTypeEnumGInput] = ListValidator(
        DataclassValidator(ServiceDisruptionTypeEnumGInput)
    )
    ...
    sitServiceDisruptionExtensionG: ExtensionTypeGInput | UnsetValueType = (
        DataclassValidator(ExtensionTypeGInput),
        Default(UnsetValue),
    )
