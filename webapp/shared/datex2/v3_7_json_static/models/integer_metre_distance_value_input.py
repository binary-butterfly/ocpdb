...
@validataclass
class IntegerMetreDistanceValueInput(ValidataclassMixin):
    """
    A measured or calculated value of distance in whole metres.
    """

    integerMetreDistance: int = IntegerValidator(min_value=0)
    comDataValueExtensionG: ExtensionTypeGInput | UnsetValueType = (
        DataclassValidator(ExtensionTypeGInput),
        Default(UnsetValue),
    )
    comIntegerMetreDistanceValueExtensionG: ExtensionTypeGInput | UnsetValueType = (
        DataclassValidator(ExtensionTypeGInput),
        Default(UnsetValue),
    )
