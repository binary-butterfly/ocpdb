...
def test_get_ocpi_22_connectors_strict(
    db: SQLAlchemy,
    ...
    response = public_test_client.get(
        path='/api/public/ocpi/2.2/connectors?strict=true',
    )
...
def test_get_ocpi_22_connector_strict(
    db: SQLAlchemy,
    public_test_client: OpenApiFlaskClient,
) -> None:
    db.session.add(
    ...
    response = public_test_client.get(
        path='/api/public/ocpi/2.2/connectors/1?strict=true',
    )
    assert response.status_code == HTTPStatus.OK
...
def test_get_ocpi_22_connectors_by_source_uid(
    db: SQLAlchemy,
    public_test_client: OpenApiFlaskClient,
) -> None:
    db.session.add_all([
    ...
    response = public_test_client.get(
        path=f'/api/public/ocpi/2.2/connectors?strict=true&source_uid={SOURCE_UID_1}',
    )
    assert response.status_code == HTTPStatus.OK
...
def test_get_ocpi_22_connectors_by_evse_id(
    db: SQLAlchemy,
    public_test_client: OpenApiFlaskClient,
) -> None:
    db.session.add(
    ...
    response = public_test_client.get(
        path='/api/public/ocpi/2.2/connectors?strict=true&evse_id=1',
    )
    assert response.status_code == HTTPStatus.OK
...
def test_get_ocpi_22_connector_non_strict(
    db: SQLAlchemy,
    ...
    response = test_client.get(
        path='/api/public/ocpi/2.2/connectors/1',
    )
    assert response.status_code == HTTPStatus.OK
    data = response.json
    assert 'id' in data
