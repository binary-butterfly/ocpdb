...
def test_get_ocpi_22_evses_strict(
    ...
    response = public_test_client.get(
        path='/api/public/ocpi/2.2/evses?strict=true',
    )
...
def test_get_ocpi_22_evse_strict(
    ...
    response = public_test_client.get(
        path='/api/public/ocpi/2.2/evses/1?strict=true',
    )
    assert response.status_code == HTTPStatus.OK
...
def test_get_ocpi_22_evses_by_status(
    ...
    response = public_test_client.get(
        path='/api/public/ocpi/2.2/evses?strict=true&status=AVAILABLE',
    )
    assert response.status_code == HTTPStatus.OK
...
def test_get_ocpi_22_evses_by_source_uid(
    ...
    response = public_test_client.get(
        path=f'/api/public/ocpi/2.2/evses?strict=true&source_uid={SOURCE_UID_1}',
    )
    assert response.status_code == HTTPStatus.OK
    assert response.json['total_count'] == 2
    assert len(response.json['items']) == 2
...
def test_get_ocpi_22_evse_non_strict(
    ...
    response = test_client.get(
        path='/api/public/ocpi/2.2/evses/1',
    )
    assert response.status_code == HTTPStatus.OK
    data = response.json
    assert 'uid' in data
