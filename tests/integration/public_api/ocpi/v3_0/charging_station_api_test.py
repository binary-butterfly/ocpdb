...
def test_get_ocpi_30_charging_stations_strict(
    ...
    response = public_test_client.get(
        path='/api/ocpi/3.0/charge-stations?strict=true',
    )
...
def test_get_ocpi_30_charging_station_strict(
    ...
    response = public_test_client.get(
        path='/api/ocpi/3.0/charge-stations/1?strict=true',
    )
    assert response.status_code == HTTPStatus.OK
...
def test_get_ocpi_30_charging_stations_by_source_uid(
    ...
    response = public_test_client.get(
        path=f'/api/ocpi/3.0/charge-stations?strict=true&source_uid={SOURCE_UID_1}',
    )
    assert response.status_code == HTTPStatus.OK
    assert response.json['total_count'] == 2
    assert len(response.json['items']) == 2
...
def test_get_ocpi_30_charging_stations_by_location_id(
    ...
    response = public_test_client.get(
        path='/api/ocpi/3.0/charge-stations?strict=true&location_id=1',
    )
    assert response.status_code == HTTPStatus.OK
...
def test_get_ocpi_30_charging_station_non_strict(
    ...
    response = test_client.get(
        path='/api/ocpi/3.0/charge-stations/1',
    )
    assert response.status_code == HTTPStatus.OK
    data = response.json
    assert 'uid' in data
