...
def test_get_ocpi_30_businesses(
    db: SQLAlchemy,
...
def test_get_ocpi_30_business(
    ...
    response = public_test_client.get(
        path='/api/ocpi/3.0/businesses/1',
    )
    assert response.status_code == HTTPStatus.OK
    assert response.json == {'name': BUSINESS_1_NAME}


def test_get_ocpi_30_businesses_by_name(
    db: SQLAlchemy,
    public_test_client: OpenApiFlaskClient,
) -> None:
    db.session.add_all([get_business_1(), get_business_2()])
    ...
    response = public_test_client.get(
        path='/api/ocpi/3.0/businesses?name=Electro',
    )
    assert response.status_code == HTTPStatus.OK
...
def test_get_ocpi_30_business_with_website(
    db: SQLAlchemy,
    public_test_client: OpenApiFlaskClient,
) -> None:
