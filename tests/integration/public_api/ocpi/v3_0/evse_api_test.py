...
def test_get_ocpi_30_evses_strict(
    db: SQLAlchemy,
    ...
    response = public_test_client.get(
        path='/api/ocpi/3.0/evses?strict=true',
    )
...
def test_get_ocpi_30_evse_strict(
    db: SQLAlchemy,
    public_test_client: OpenApiFlaskClient,
) -> None:
    db.session.add(
    ...
    response = public_test_client.get(
        path='/api/ocpi/3.0/evses/1?strict=true',
    )
    assert response.status_code == HTTPStatus.OK
...
def test_get_ocpi_30_evses_by_status(
    db: SQLAlchemy,
    public_test_client: OpenApiFlaskClient,
) -> None:
    db.session.add_all([
    ...
    response = public_test_client.get(
        path='/api/ocpi/3.0/evses?strict=true&status=AVAILABLE',
    )
    assert response.status_code == HTTPStatus.OK
...
def test_get_ocpi_30_evses_by_source_uid(
    db: SQLAlchemy,
    public_test_client: OpenApiFlaskClient,
) -> None:
    db.session.add_all([
    ...
    response = public_test_client.get(
        path=f'/api/ocpi/3.0/evses?strict=true&source_uid={SOURCE_UID_1}',
    )
    assert response.status_code == HTTPStatus.OK
    assert response.json['total_count'] == 2
    assert len(response.json['items']) == 2
...
def test_get_ocpi_30_evse_non_strict(
    db: SQLAlchemy,
