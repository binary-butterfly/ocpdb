...
def test_get_ocpi_30_connectors_strict(
    ...
    response = public_test_client.get(
        path='/api/ocpi/public/3.0/connectors?strict=true',
    )
...
def test_get_ocpi_30_connector_strict(
    ...
    response = public_test_client.get(
        path='/api/ocpi/public/3.0/connectors/1?strict=true',
    )
    assert response.status_code == HTTPStatus.OK
...
def test_get_ocpi_30_connectors_by_source_uid(
    ...
    response = public_test_client.get(
        path=f'/api/ocpi/public/3.0/connectors?strict=true&source_uid={SOURCE_UID_1}',
    )
    assert response.status_code == HTTPStatus.OK
...
def test_get_ocpi_30_connectors_by_evse_id(
    ...
    response = public_test_client.get(
        path='/api/ocpi/public/3.0/connectors?strict=true&evse_id=1',
    )
    assert response.status_code == HTTPStatus.OK
...
def test_get_ocpi_30_connector_non_strict(
    ...
    response = test_client.get(
        path='/api/ocpi/public/3.0/connectors/1',
    )
    assert response.status_code == HTTPStatus.OK
    data = response.json
    assert 'id' in data
